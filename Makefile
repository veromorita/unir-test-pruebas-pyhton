...
test-unit:
	docker run --rm --volume "$(PROJECT_DIR):/opt/calc" --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest pytest "$(PROJECT_DIR)/test/unit" --cov=app --cov-report=xml:results/coverage.xml --cov-report=html:results/coverage --junit-xml=results/unit_result.xml -m unit || true
...
test-api:
	...
	docker run --rm --volume "$(PROJECT_DIR):/opt/calc" --network calc-test-api --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver:5000/ -w /opt/calc calculator-app:latest pytest "$(PROJECT_DIR)/test/rest" --junit-xml=results/api_result.xml -m api || true
