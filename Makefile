...
PROJECT_DIR := C:/Program Files/Jenkins/workspace/Laboratorio3
...
test-unit:
	docker run --rm --volume $(PROJECT_DIR):/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest pytest test/unit --cov=app --cov-report=xml:results/coverage.xml --cov-report=html:results/coverage --junit-xml=results/unit_result.xml -m unit || true
...
test-api:
	...
	docker run --rm --volume $(PROJECT_DIR):/opt/calc --network calc-test-api --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver:5000/ -w /opt/calc calculator-app:latest pytest test/rest --junit-xml=results/api_result.xml -m api || true
...
test-e2e:
	...
	docker run --rm --volume $(PROJECT_DIR)/test/e2e/cypress.json:/cypress.json --volume $(PROJECT_DIR)/test/e2e/cypress:/cypress --volume $(PROJECT_DIR)/results:/results  --network calc-test-e2e cypress/included:4.9.0 --browser chrome || true
...
test-e2e-wiremock:
	...
	docker run --rm --volume $(PROJECT_DIR)/test/e2e/cypress.json:/cypress.json --volume $(PROJECT_DIR)/test/e2e/cypress:/cypress --volume $(PROJECT_DIR)/results:/results --network calc-test-e2e-wiremock cypress/included:4.9.0 --browser chrome || true
...
zap-scan:
	...
	sleep 10
	docker run --rm --volume $(PROJECT_DIR):/opt/calc --network calc-test-zap --env PYTHONPATH=/opt/calc --env ZAP_API_KEY=$(ZAP_API_KEY) --env ZAP_API_URL=$(ZAP_API_URL) --env TARGET_URL=$(ZAP_TARGET_URL) -w /opt/calc calculator-app:latest pytest --junit-xml=results/sec_result.xml -m security  || true
